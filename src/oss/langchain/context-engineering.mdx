...
- **Data Sources**: Read from State (session), Store (persistent), or Runtime Context (config)
- **Middleware**: Intercept the agent loop to make decisions
- **Control Knobs**: Dynamically adjust Prompt, Messages, Tools, Model, or output Format

```mermaid
flowchart TD
    State[State<br/>short-term memory]
    Store[Store<br/>long-term memory]
    Runtime[Runtime Context<br/>static configuration]

    State --> Middleware{Middleware}
    Store --> Middleware
    Runtime --> Middleware

    Middleware --> Prompt[System Prompt]
    Middleware --> Messages[Messages]
    Middleware --> Tools[Tools]
    Middleware --> Model[Model]
    Middleware --> Format[Response Format]
```
...
At each stage of the agent loop, you can read from (and sometimes write to) different data sources. LangChain uses [middleware](/oss/langchain/middleware) to give you fine-grained control over context at each stage. Click a card below to see detailed examples:
