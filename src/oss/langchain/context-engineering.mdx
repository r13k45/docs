...
LangChain uses [middleware](/oss/langchain/middleware) to give you fine-grained control over context at each stage of the agent loop. All the examples below use middleware to demonstrate context engineering patterns.

## Quick start

Here are the most common context engineering patterns to get you started:

<CardGroup cols={2}>
  <Card title="Personalize with user preferences" icon="user">
    Use **Store** to fetch user preferences and customize the agent's behavior based on long-term memory
    ...
    def personalized_prompt(request: ModelRequest) -> str:
        ...
        return f"You are helpful. User prefers {prefs.value['style']} responses."
    ```
  </Card>

  <Card title="Trim long conversations" icon="scissors">
    Use **State** to manage conversation length and keep context within model limits

    ```python
    @before_model
    def trim_messages(state: AgentState, runtime: Runtime) -> dict | None:
        if len(state["messages"]) > 10:
            return {"messages": [state["messages"][0], *state["messages"][-8:]]}
    ```
  </Card>

  <Card title="Control access by role" icon="shield-halved">
    Use **Runtime Context** to filter tools based on user permissions

    ```python
    @wrap_model_call
    def filter_by_role(request: ModelRequest, handler) -> ModelResponse:
  ...
  <Card title="Filter sensitive data" icon="eye-slash">
    Use **Transient Messages** to remove sensitive information before sending to LLM

    ```python
    @wrap_model_call
    def filter_sensitive(request: ModelRequest, handler) -> ModelResponse:
        request.messages = [m for m in request.messages if "password" not in str(m).lower()]
        return handler(request)
    ```
  </Card>
</CardGroup>

## Context engineering by stage

At each stage of the agent loop, you can read from (and sometimes write to) different data sources. Click a card below to see detailed examples:

<CardGroup cols={3}>
  <Card title="Configure System Prompt" icon="comment-dots" href="#configure-system-prompt">
    Dynamically set the system prompt based on State, Store, or Runtime Context

    **Access**: State (read), Store (read), Runtime Context (read), Transient Messages (read)
  </Card>

  <Card title="Configure Messages" icon="messages" href="#configure-messages">
    Trim, summarize, or transform messages before sending to the LLM

    **Access**: State (read/write), Store (read/write), Runtime Context (read), Transient Messages (read/write)
  </Card>

  <Card title="Configure Tools" icon="wrench" href="#configure-tools">
    Dynamically select which tools are available based on context

    **Access**: State (read), Store (read), Runtime Context (read)
  </Card>

  <Card title="Configure Model" icon="microchip" href="#configure-model">
    Switch between models based on conversation length, user preferences, or cost constraints

    **Access**: State (read), Store (read), Runtime Context (read)
  </Card>

  <Card title="Configure Structured Output" icon="table" href="#configure-structured-output">
  ...
  <Card title="Execute Tools" icon="play" href="#execute-tools">
    ...
    **Access**: State (read/write), Store (read/write), Runtime Context (read)
  </Card>
</CardGroup>
...
<Tabs>
  ...
  <Tab title="Transient Messages">
    Read transient messages to adjust the system prompt:
