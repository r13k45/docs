...
This loop continues until the LLM decides to finish. Throughout this process, the agent may access different sources of data:
...
<CardGroup cols={3}>
  <Card title="State" icon="hard-drive">
    ...
    **Examples**: messages, uploaded files, todos, status flags, tool results
  </Card>
...
To have full control over your agent, you need to control what happens in each node:
...
The model node is where the LLM is called. You have full control over what gets passed to the LLM at this stage. Using [middleware](/oss/langchain/middleware), you can dynamically adjust:

- [**System prompt**](/oss/langchain/middleware#system-prompt): Set instructions based on State, Store, or Runtime Context
- [**Messages**](/oss/langchain/middleware#messages): Trim, summarize, or transform the conversation history
- [**Tools**](/oss/langchain/middleware#tools): Dynamically select which tools are available
- [**Model**](/oss/langchain/middleware#model): Switch between different models based on context
- [**Response format**](/oss/langchain/middleware#response-format): Control structured output schemas
