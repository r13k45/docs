...
Context engineering is about building dynamic systems to provide the right information and tools in the right format so the LLM can accomplish the task.

When building agents with LangChain, this means:
* Managing **data sources**: State, Store, and Runtime Context
* Using **middleware** to adjust the context at each step
* Updating the **context** of the agent at each step

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '20px', 'primaryColor': '#efedfe', 'primaryBorderColor': '#c4b5fd'}}}%%
flowchart TD
    State(["<b style='font-size: 18px'>State</b><br/><span style='font-size: 14px; margin-top: -5px'>short-term memory</span>"])
    Store(["<b style='font-size: 18px'>Store</b><br/><span style='font-size: 14px; margin-top: -5px'>long-term memory</span>"])
    Runtime(["<b style='font-size: 18px'>Runtime Context</b><br/><span style='font-size: 14px; margin-top: -5px'>static configuration</span>"])

    State --> Middleware{{"<b style='font-size: 18px'>Middleware</b>"}}
    Store --> Middleware
    Runtime --> Middleware

    Middleware --> Prompt(["<b style='font-size: 18px'>System Prompt</b>"])
    Middleware --> Messages(["<b style='font-size: 18px'>Messages</b>"])
    Middleware --> Tools(["<b style='font-size: 18px'>Tools</b>"])
    Middleware --> Model(["<b style='font-size: 18px'>Model</b>"])
    Middleware --> Format(["<b style='font-size: 18px'>Response Format</b>"])

    style State fill:#efedfe,stroke:#c4b5fd,stroke-width:3px,padding:20px
    style Store fill:#efedfe,stroke:#c4b5fd,stroke-width:3px,padding:20px
    style Runtime fill:#efedfe,stroke:#c4b5fd,stroke-width:3px,padding:20px
    style Middleware fill:#ffffff,stroke:#000000,stroke-width:3px,padding:20px
    style Prompt fill:#e6f4f1,stroke:#7dd3c0,stroke-width:3px,padding:20px
    style Messages fill:#e6f4f1,stroke:#7dd3c0,stroke-width:3px,padding:20px
    style Tools fill:#e6f4f1,stroke:#7dd3c0,stroke-width:3px,padding:20px
    style Model fill:#e6f4f1,stroke:#7dd3c0,stroke-width:3px,padding:20px
    style Format fill:#e6f4f1,stroke:#7dd3c0,stroke-width:3px,padding:20px
```

### Data Sources

<CardGroup cols={3}>
  <Card title="State" icon="hard-drive">
    **Short-term memory**

    Conversation-scoped memory.

    **Examples**: messages, uploaded files, authentication status
  </Card>
  ...
  <Card title="Store" icon="database">
    ...
    **Examples**: user preferences, extracted insights, memories, historical data
  </Card>

  <Card title="Runtime Context" icon="gear">
    **Static configuration**

    Conversation-scoped configuration.

    **Examples**: user ID, API keys, database connections, permissions
  </Card>
</CardGroup>

### Context

At each stage of the agent loop, you can read from (and sometimes write to) different data sources. LangChain uses [middleware](/oss/langchain/middleware) to give you fine-grained control over context at each stage.
Click a card below to see detailed examples:
