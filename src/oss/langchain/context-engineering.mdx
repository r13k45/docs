...
A typical agent loop consists of two steps:

1. **Model call** - calls the LLM, and either returns the response or routes to tool execution
...
[Middleware](/oss/langchain/middleware) provides additional hooks that run **between** these steps, allowing you to modify context as it flows through the agent loop. This enables persisting changes to messages, or other context, between the model call and tool execution.

To have full control over your agent, you need to control what happens at each step and between them:

| Context Type | What You Control | Examples |
|--------------|------------------|----------|
| **[Model Context](#model-context)** | What goes into the model call | System prompt, messages, tools, model selection, response format |
| **[Tool Context](#tool-context)** | What happens inside tool execution | Reading from and writing to state, store, and runtime context |
| **[In-Between Context](#in-between-context)** | What happens between these steps | Summarize conversations, redact PII, modify context flow |
...
This is where you control what goes into the model call. You have full control over what gets passed to the LLM.

**Important:** Changes made here affect **transient context** - the information sent to the LLM for a single call. These modifications don't change what's saved in state (your conversation history). To modify what gets saved to state, use [middleware hooks](#in-between-context).
...
This is where you control what happens inside tool execution. Tools are executed after the LLM requests them. Tools are unique in that they both consume (read) context and produce (write) context that flows back to the model call.
...
This is where you control what happens between the model call and tool execution. Middleware provides hooks to modify context as it flows through your agent loop. Unlike model and tool contexts which operate within their respective steps, middleware intercepts and transforms data **between** these steps.
