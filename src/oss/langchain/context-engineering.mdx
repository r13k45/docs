...
LangChain [middleware](/oss/langchain/middleware) is the mechanism that makes context engineering practical. It intercepts and transforms context at every step of the agent loop - adjusting prompts, filtering tools, managing conversation history, and controlling what gets persisted to state.
...
[Middleware](/oss/langchain/middleware) provides hooks that run between model calls and tool execution, with three key capabilities:
