...
Context engineering is about reading from **data sources** and using **middleware** to dynamically adjust the **control knobs** of your agent:

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
    State["<b>State</b><br/>short-term memory"]
    Store["<b>Store</b><br/>long-term memory"]
    Runtime["<b>Runtime Context</b><br/>static configuration"]

    State --> Middleware{{"<b>Middleware</b>"}}
    Store --> Middleware
    Runtime --> Middleware

    Middleware --> Prompt["<b>System Prompt</b>"]
    Middleware --> Messages["<b>Messages</b>"]
    Middleware --> Tools["<b>Tools</b>"]
    Middleware --> Model["<b>Model</b>"]
    Middleware --> Format["<b>Response Format</b>"]

    style State fill:#efedfe,stroke:#7c3aed,stroke-width:3px
    style Store fill:#efedfe,stroke:#7c3aed,stroke-width:3px
    style Runtime fill:#efedfe,stroke:#7c3aed,stroke-width:3px
    style Middleware fill:#efedfe,stroke:#7c3aed,stroke-width:3px
    style Prompt fill:#e6f4f1,stroke:#2f6868,stroke-width:3px
    style Messages fill:#e6f4f1,stroke:#2f6868,stroke-width:3px
    style Tools fill:#e6f4f1,stroke:#2f6868,stroke-width:3px
    style Model fill:#e6f4f1,stroke:#2f6868,stroke-width:3px
    style Format fill:#e6f4f1,stroke:#2f6868,stroke-width:3px
```
...
- Use **State** for data that changes during a conversation (authentication status, file uploads, conversation stage)
- Use **Store** for data that persists across conversations (user preferences, learned facts, historical summaries)
- Use **Runtime Context** for configuration that stays constant during a run (user ID, API keys, environment settings, permissions)
</Tip>
