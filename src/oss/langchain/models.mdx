...
<Tip>
    ...
    A full list of async methods can be found in the [reference docs](https://reference.langchain.com/python/langchain/).
</Tip>
:::
...
```
<Note>
    When using @[`batch_as_completed()`][BaseChatModel.batch_as_completed], results may arrive out of order. Each includes the input index for matching to reconstruct the original order if needed.
</Note>
